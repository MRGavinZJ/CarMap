...
- (void)initRoute {
    ...
    [self.car2 setCoordinate:s_coords[20]];
}
...
//    double speed_car1 = 120.0 / 3.6; //80 km/h
    int count = sizeof(s_coords) / sizeof(s_coords[0]);
    [self.car1 setCoordinate:s_coords[0]];
    [self.car1 addMoveAnimationWithKeyCoordinates:s_coords count:count withDuration:3 withName:nil completeCallback:^(BOOL isFinished) {
        ;
    }];
...
//        NSNumber *num = [self.distanceArray objectAtIndex:i - 1];
        ...
        [self.car2 addMoveAnimationWithKeyCoordinates:&s_coords[20] count:count-20 withDuration:3 withName:nil completeCallback:^(BOOL isFinished) {
